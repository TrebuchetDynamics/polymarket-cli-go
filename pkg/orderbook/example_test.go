...
// token ID. A test HTTP server stands in for the production CLOB so the example
// is hermetic and runnable with `go test ./pkg/orderbook`.
func Example_orderBook() {
	...
	reader := orderbook.NewReader(server.URL)
