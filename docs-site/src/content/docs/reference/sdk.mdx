...
requirement. Gamma and Data API DTOs have been promoted: `pkg/gamma`,
`pkg/data`, and the corresponding methods on `pkg/universal` return
`pkg/types`.
...
| Public type ownership | Gamma and Data API DTOs are public in `pkg/types`; CLOB, enrichment, and stream signatures still need promotion work before claiming a fully clean external SDK |
