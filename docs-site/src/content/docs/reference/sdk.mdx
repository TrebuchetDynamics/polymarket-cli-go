...
`internal/polytypes`, `internal/clob`, `internal/stream`, or
`internal/transport`. That is an SDK hygiene gap, not an upstream Polymarket
requirement. Gamma, Data API, and read-only CLOB DTOs have been promoted:
`pkg/gamma`, `pkg/data`, `pkg/clob`, and the corresponding read methods on
`pkg/universal` return `pkg/types`.
...
| Public type ownership | Gamma, Data API, and read-only CLOB DTOs are public in `pkg/types`; authenticated CLOB account/trading, rewards, enrichment, and stream signatures still need promotion work before claiming a fully clean external SDK |
