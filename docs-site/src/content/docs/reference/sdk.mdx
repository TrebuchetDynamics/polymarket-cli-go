...
`internal/polytypes`, `internal/clob`, or `internal/transport`. That is an SDK
hygiene gap, not an upstream Polymarket requirement. Gamma, Data API,
read-only CLOB DTOs, and market WebSocket DTOs have been promoted:
`pkg/gamma`, `pkg/data`, `pkg/clob`, `pkg/stream`, and the corresponding read
...
## `pkg/clob` — Read-Only CLOB Market Data
...
| Public type ownership | Gamma, Data API, and read-only CLOB DTOs are public in `pkg/types`; market WebSocket DTOs are public in `pkg/stream`; authenticated CLOB account/trading, rewards, enrichment, and user-stream signatures still need promotion work before claiming a fully clean external SDK |
