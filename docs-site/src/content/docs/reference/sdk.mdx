...
`internal/polytypes`, `internal/clob`, or `internal/stream`. That is an SDK
hygiene gap, not an upstream Polymarket requirement. Data API DTOs have been
promoted first: `pkg/data` and the Data API methods on `pkg/universal` return
`pkg/types`.
...
| Public type ownership | Data API DTOs are public in `pkg/types`; Gamma, CLOB, and stream signatures still need promotion work before claiming a fully clean external SDK |
