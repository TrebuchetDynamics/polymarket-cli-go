...
Public packages for consumption by `go-bot` and other Go projects. These are
the stable API contracts: what callers can depend on across minor versions.
...
| Gap | Current Status |
|---|---|
| Deposit-wallet deploy / approve / batch SDK | Implemented under `internal/relayer`, `internal/wallet`, and CLI commands, but not promoted to a public `pkg/` package |
