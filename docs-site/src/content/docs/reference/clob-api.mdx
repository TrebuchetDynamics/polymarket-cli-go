...
For `signatureType = 3` (`POLY_1271`), `maker` is the deposit wallet and
`signer` is the owner EOA, matching the current upstream client shape. The owner
EOA signs an ERC-7739 `TypedDataSign` wrapper that the deposit wallet validates
through ERC-1271. Polygolem's wrapped signature is `0x` plus 634 hex chars:
