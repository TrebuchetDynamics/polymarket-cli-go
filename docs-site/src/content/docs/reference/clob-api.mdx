...
Polygolem wraps CLOB in `internal/clob`. The public Go SDK exposes the supported
surface through `pkg/universal` and read-only order books through
`pkg/bookreader`.
...
polygolem clob create-order \
  --token <token-id> --side buy --price 0.5 --size 10 --signature-type deposit
