...
| CLOB placement | `internal/clob`, `pkg/clob`, `pkg/universal` | `clob create-order`, `clob market-order`, `--builder-code` | Deposit wallet only |
...
| CLOB batch placement | `internal/clob`, `pkg/clob`, `pkg/universal` | Not exposed | SDK covered; CLI gap |
...
- Batch order placement is exposed through the SDK; a CLI wrapper is still a
  gap.
