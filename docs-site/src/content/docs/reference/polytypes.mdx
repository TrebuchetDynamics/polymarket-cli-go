...
The `internal/polytypes` package defines the protocol-level types shared across
polygolem clients. These types are part of the current method signatures in
`pkg/gamma` and `pkg/universal`; promoting or re-exporting them under a public
package path is a known SDK cleanup gap.
