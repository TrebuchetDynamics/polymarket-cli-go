...
| **`pkg/`** | 10 | Public Go SDK contracts. Gamma and Data API DTOs use public `pkg/types`; `pkg/bookreader` is deprecated in favor of `pkg/orderbook`; CLOB/enrichment/stream type paths still need cleanup. |
...
internal/{wallet, orders, execution, risk, paper, marketdiscovery}
        ^
        |
pkg/{universal, gamma, data, types, orderbook, bookreader, marketresolver, bridge, relayer, pagination}
```
