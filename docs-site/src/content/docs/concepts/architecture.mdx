...
| **`pkg/`** | 12 | Public Go SDK contracts. Gamma, Data API, and read-only CLOB DTOs use public `pkg/types`; market WebSocket events use public `pkg/stream` DTOs; `pkg/bookreader` is deprecated in favor of `pkg/orderbook`; enrichment and authenticated type paths still need cleanup. |
...
internal/{wallet, orders, execution, risk, paper, marketdiscovery}
        ^
        |
pkg/{universal, gamma, data, types, clob, stream, orderbook, bookreader, marketresolver, bridge, relayer, pagination}
```
