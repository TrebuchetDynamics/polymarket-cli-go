...
| **`pkg/`** | 6 | Stable public Go SDK. Semver-stable contracts. |
| **`internal/`** | 21 | Implementation. Not part of public API. |

Dependencies flow **inward only**: `cmd → internal → pkg`.
...
internal/{wallet, orders, execution, risk, paper, marketdiscovery}
        |
pkg/{universal, gamma, bookreader, marketresolver, bridge, pagination}
```
