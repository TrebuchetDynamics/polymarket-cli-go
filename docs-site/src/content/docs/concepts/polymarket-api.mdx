...
**Authenticated. Requires Builder API Key or Relayer API Key.**

Gas-sponsored transaction submission for wallet lifecycle operations.
...
| API Keys | List relayer API keys (Gamma auth required) |

The relayer pays all gas for deploy, approve, and trade operations. Users only pay gas for the initial pUSD transfer to their deposit wallet.
