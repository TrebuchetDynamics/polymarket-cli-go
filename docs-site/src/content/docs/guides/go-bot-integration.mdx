...
`pkg/types`; public market stream DTOs are public in `pkg/stream`.
Authenticated CLOB account/trading, rewards, enrichment, and user-stream
signatures still expose implementation types from `internal/*`; treat those
paths as a documented SDK cleanup gap.
</Aside>
