...
- One-call live onboarding. `pkg/relayer` exposes wallet lifecycle primitives,
  and `pkg/universal` exposes CLOB order placement and cancellation, but the
  full `deploy -> approve -> fund -> sync -> preflight` operator flow remains
  a CLI workflow.
- Configured V2 `builderCode` order attribution. Polygolem currently signs
  orders with `builder = bytes32(0)`.
