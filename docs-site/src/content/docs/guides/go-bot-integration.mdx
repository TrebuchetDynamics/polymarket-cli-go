...
<Aside type="note">
Stability note: the exported methods under `pkg/` are the intended public
surface, but some method signatures still expose implementation types from
`internal/*`. Treat those type paths as a documented SDK cleanup gap.
</Aside>
