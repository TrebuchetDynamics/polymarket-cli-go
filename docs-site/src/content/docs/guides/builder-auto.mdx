---
title: Headless V2 Onboarding
description: Create CLOB L2 credentials, mint V2 relayer keys, deploy a deposit wallet, and attach builder attribution without a browser.
---

Polygolem splits the V2 setup into three credential systems:

- `builder auto` creates CLOB L2 credentials at `clob.polymarket.com/auth/api-key`.
...
`{ apiKey, secret, passphrase }`. After the deposit wallet exists,
`clob create-api-key-for-address --owner <deposit-wallet>` creates the
deposit-wallet-owned key used by balances, orders, trades, and cancellations.
That owner-scoped flow sets the ClobAuth `address` field to the deposit wallet
and wraps the EOA signature for ERC-1271 validation.
