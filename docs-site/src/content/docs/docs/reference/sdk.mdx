...
must `errors.Is` and stop — the V2 deposit-wallet redeem path is
non-negotiable; there is no fallback to attempt.
...
if err != nil {
    if errors.Is(err, relayer.ErrRelayerAllowlistBlocked) {
        // Surface as upstream block (Polymarket/builder-relayer-client#29) and stop.
        return err
    }
    return err
}
...
const CtfCollateralAdapter = "0xADa100874d00e3331D00F2007a9c336a65009718"
const NegRiskCtfCollateralAdapter = "0xAdA200001000ef00D07553cEE7006808F895c6F1"
...
as a structured upstream block — there is no safe fallback for V2
deposit-wallet positions. Tracking: `Polymarket/builder-relayer-client#29`
(closed without response on 2026-05-06).
