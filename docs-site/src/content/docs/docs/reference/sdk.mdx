...
## `pkg/settlement` — V2 Winner Redemption (Planned)
...
```go
func FindRedeemable(ctx context.Context, data *data.Client, owner string) ([]RedeemablePosition, error)
func BuildRedeemCall(p RedeemablePosition) (relayer.DepositWalletCall, error)
func SubmitRedeem(ctx context.Context, rc *relayer.Client, privateKey string, positions []RedeemablePosition) (*relayer.RelayerTransaction, error)
```
