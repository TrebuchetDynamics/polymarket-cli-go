...
polygolem clob market-by-token <token-id>
```
...
- Recent relayer transaction list and Bridge quote/status CLI commands are
  documented on their own reference pages because they belong to different
  Polymarket API hosts.
