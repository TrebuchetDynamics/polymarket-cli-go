...
Or resolve the current window directly:
...
### 2. One-command paper trade

Resolve the current window, fetch live price, and trade in a single step:
...
This is the fastest path from idea to paper position — no manual token ID lookup.
