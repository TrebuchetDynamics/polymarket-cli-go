...
For live bot feeds, use `marketdata live` instead of stitching raw stream
messages yourself. It subscribes to the same public market WebSocket as
`stream market`, tracks each token's latest book, and emits enriched snapshots
with `best_bid`, `best_ask`, `spread`, `midpoint`, `tick_size`,
`last_trade_price`, `bids`, and `asks`. It requests V2 custom feature events by
default so `best_bid_ask` updates can refresh top-of-book snapshots without a
full book delta.
