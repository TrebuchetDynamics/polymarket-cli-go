...
Before a live bot buys another market, prove the deposit wallet can also settle
winning positions through the official V2 path:
...
The check is read-only. It verifies deposit-wallet bytecode, relayer
credentials supplied by the caller, Data API reachability, and CTF approvals
for both V2 collateral adapters.
