...
- `auth login` performs SIWE login and mints `RELAYER_API_KEY` +
...
Wallet commands can run `auth login` automatically when relayer credentials
are missing. Run `polygolem auth login` yourself when you want to inspect or
