...
| **`pkg/`** | 12 | Public Go SDK contracts. Gamma, Data API, and read-only CLOB DTOs use public `pkg/types`; authenticated CLOB account/order DTOs live in `pkg/clob`; market WebSocket events use public `pkg/stream` DTOs; `pkg/bookreader` is deprecated in favor of `pkg/orderbook`; enrichment and rewards type paths still need cleanup. |
