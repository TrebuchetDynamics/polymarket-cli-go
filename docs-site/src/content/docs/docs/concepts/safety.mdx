...
the allowed list", stop. The production factory `proxy()` entrypoint is
`onlyOperator`, so there is no direct owner-EOA fallback, and raw
`ConditionalTokens.redeemPositions` is not the V2 pUSD-native redeem path.
