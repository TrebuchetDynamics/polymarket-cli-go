...
for V2 pUSD-native flows. Standard binary-market split, merge, and redeem
calls route through `CtfCollateralAdapter`; negative-risk calls route through
...
| Adapter approve batch | Polymarket relayer | FREE |
...
| CTF split/merge/redeem through V2 collateral adapters | Polymarket relayer | FREE |
