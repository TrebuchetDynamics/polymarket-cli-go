...
batch as not allowlisted, verify the adapter addresses against Polymarket's
current contracts reference first. Stale adapter constants caused the
2026-05-09 live blocker; current addresses plus the same rejection is the
actual upstream-blocked state.
