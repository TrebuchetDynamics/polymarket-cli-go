...
|---|---|---|---|---|---|
| Gamma markets | Search, list markets, fetch market by ID/slug, enrich with CLOB data | `internal/gamma`, `pkg/universal` | `discover search`, `discover markets`, `discover market`, `discover enrich` | `README.md`, `docs/COMMANDS.md`, Starlight CLI/Gamma pages | Gamma `httptest` tests, CLI command registration |
| Gamma taxonomy | Tags/categories, series, comments | `internal/gamma`, `pkg/universal` | `discover tags`, `discover series`, `discover comments` | `docs/COMMANDS.md`, Starlight CLI/Gamma pages | CLI command registration |
| CLOB public data | Order book, price, midpoint, spread, tick size, fee rate, last trade, price history, market list, market lookup | `internal/clob`, `pkg/bookreader`, `pkg/universal` | `orderbook *`, `clob book`, `clob tick-size`, `clob market`, `clob markets`, `clob price-history` | `README.md`, `docs/COMMANDS.md`, Starlight CLOB/CLI pages | CLOB `httptest` tests, CLI command registration |
...
| Data API | Positions, closed positions, trades, activity, holders, value, markets traded, open interest, leaderboard, live volume | `internal/dataapi`, `pkg/universal` | `data positions`, `data closed-positions`, `data trades`, `data activity`, `data holders`, `data value`, `data markets-traded`, `data open-interest`, `data leaderboard`, `data live-volume` | `README.md`, `docs/COMMANDS.md`, Starlight Data/CLI pages | Universal Data API route tests, CLI command registration |
...
| WebSocket market stream | Public market channel subscription, book/price/last-trade dispatch, reconnect, dedup helpers | `internal/stream`, `pkg/universal` stream constructor | `stream market` | `README.md`, `docs/COMMANDS.md`, Starlight Stream/CLI pages | Local WebSocket CLI test |
