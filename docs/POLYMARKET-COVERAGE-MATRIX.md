...
| CLOB public data | Order book, price, midpoint, spread, tick size, fee rate, last trade, price history, market list, condition lookup, token lookup | `internal/clob`, `pkg/clob`, `pkg/orderbook`, `pkg/types`, `pkg/universal` | `orderbook *`, `clob book`, `clob tick-size`, `clob market`, `clob market-by-token`, `clob markets`, `clob price-history` | `README.md`, `docs/COMMANDS.md`, Starlight CLOB/CLI pages | CLOB `httptest` tests, public SDK boundary test, CLI command registration |
